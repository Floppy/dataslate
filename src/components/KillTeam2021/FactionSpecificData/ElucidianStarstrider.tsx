...
export const ElucidianStarstrider: FC<Props> = (props) => {
  ...
  const factionSpecificData = getFactionSpecificData('Elucidian Starstriders')

  if (factionSpecificData === undefined || factionSpecificData.name !== 'Elucidian Starstriders') {
    return <></>
  }
  ...
  return (
    <>
      ...
      {factionSpecificData.starstriderAbilities.map(x => (
        <div key={x.name} style={{ display: 'flex', justifyContent: 'space-between', flexDirection: 'row', breakBefore: 'page' }}>
          <Card style={{ width: '100%', marginRight: '5px' }}>
            <Card.Header style={{ ...headingStyle }} as='h2'>{x.name}</Card.Header>
            <Card.Body>
              <CompileDescription>{x.text}</CompileDescription>
            </Card.Body>
          </Card>
        </div>
      ))}
