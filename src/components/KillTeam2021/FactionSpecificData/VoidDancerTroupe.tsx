...
export const VoidDancerTroupe: FC<Props> = (props) => {
  ...
  const factionSpecificData = getFactionSpecificData('Void-Dancer Troupe')

  if (factionSpecificData === undefined || factionSpecificData.name !== 'Void-Dancer Troupe') {
    return <></>
  }
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', flexDirection: 'row', breakBefore: 'page' }}>
        <Card style={{ width: '100%', marginRight: '5px' }}>
          <Card.Header style={{ ...headingStyle }} as='h2'>{factionSpecificData.saedath.name}</Card.Header>
          <Card.Body>
            <a href={factionSpecificData.saedath.wahapediaUrl} target='_blank' rel='noreferrer'>Full Rules</a>
            <CompileDescription>{factionSpecificData.saedath.description}</CompileDescription>
          </Card.Body>
        </Card>
      </div>
    </>
